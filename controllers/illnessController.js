...
const getIllnessById = async (req, res) => {
  try {
    const { id } =req.params
    const findIllnessId = await Illness.findById(id)
...
const updateIllnessById = async (req, res) => {
  try {
    const updateIllness = await Illness.findByIdAndUpdate(req.query.illnessId, {[req.query.whatToUpdate]: req.query.update})
