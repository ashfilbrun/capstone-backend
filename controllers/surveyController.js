...
const getSurveysByUser = async (req, res) => {
  try {
    ...
    const surveysByUser = await Survey.find({ userId, date: { $eq: date } });
    if (!surveysByUser) throw Error(`Surveys not found`);
    res.json(surveysByUser);
  } catch (e) {
    console.log(e);
    res.send(`Surveys not found`);
  }
};
...
const updateSurveyById = async (req, res) => {
  try {
    const updateSurvey = await Survey.findByIdAndUpdate(req.query.SurveyId, {
